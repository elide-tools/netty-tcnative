...
// SSL_CREDENTIAL API runtime detection. tcnative compiles against upstream
// BoringSSL (which exports these), but Elide links against aws-lc, whose
// linux/musl libssl predates the delegated-credential API. Declaring the refs
// __attribute__((weak)) lets the static archive link even when the symbols are
// absent (they resolve to NULL); check_credential_api() then throws cleanly at
// runtime instead of calling through a NULL pointer. (Restores the behavior the
// elide-2.0.77 fork had; upstream 2.0.79 rewrote sslcredential.c and dropped it.)
